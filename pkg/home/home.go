...
// This helper builds paths to HPK's configuration, cache and data paths.
const lp = lazypath("hpk")

// ConfigPath returns the path where HPK stores configuration.
func ConfigPath(elem ...string) string { return lp.configPath(elem...) }

// CachePath returns the path where HPK stores cached objects.
func CachePath(elem ...string) string { return lp.cachePath(elem...) }

// DataPath returns the path where HPK stores data.
